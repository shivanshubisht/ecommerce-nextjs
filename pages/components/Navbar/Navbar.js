import Link from 'next/link'
import React from 'react'

const Navbar = () => {
    return (
        <div id="shopify-section-header" className="shopify-section">

            <header className="header-wrap" id="header-wrap">
                <nav className="header-nav">
                    <div className="container">


                        <div className="header-nav-row">


                            <div className="header-nav-toggle">
                                <button className="menu-toggle-btn header-icon" aria-haspopup="true" aria-expanded="false"
                                    aria-label="Hamburger Menu Toggle" data-menu-toggle="#mobilemenu-wrap" type="button">
                                    <i className="icon icon-menu"></i>
                                </button>
                            </div>


                            <div className="header-nav-menu header-menu-left">

                                <nav className="menu-wrap" role="navigation">
                                    <ul className="menu-list" data-depth="1">

                                        <li className="menu-item has-sub" data-type="collection_link"
                                            data-url="/collections/men" data-collapse-wrap>
                                            <div className="menu-action has-sub flex-row flex-middle">

                                                <a className="menu-link" href="/collections/men" aria-label="Men"
                                                    data-show-spinner>Men</a>
                                            </div>

                                        </li>

                                        <li className="menu-item has-sub" data-type="collection_link"
                                            data-url="/collections/women" data-collapse-wrap>
                                            <div className="menu-action has-sub flex-row flex-middle">

                                                <a className="menu-link" href="/collections/women" aria-label="Women"
                                                    data-show-spinner>Women</a>
                                            </div>
                                        </li>

                                        <li className="menu-item" data-type="page_link" data-url="/pages/front-runner-products"
                                            data-collapse-wrap>
                                            <div className="menu-action flex-row flex-middle">

                                                <a className="menu-link" href="/pages/front-runner-products" aria-label="Products"
                                                    data-show-spinner>Products</a>

                                            </div>
                                        </li>

                                        <li className="menu-item has-sub" data-type="collection_link"
                                            data-url="/collections/sale" data-collapse-wrap>
                                            <div className="menu-action has-sub flex-row flex-middle">

                                                <a className="menu-link is-sale text-bold" href="/collections/sale"
                                                    aria-label="Sale" data-show-spinner>Sale</a>
                                            </div>
                                        </li>

                                    </ul>
                                </nav>
                            </div>


                            <div className="header-nav-logo flex-1 text-center">
                                <Link href="/">
                                    <a className="logo-link" title="Storm Creek">
                                        <img width="200px" src="data:image/png;base64,"
                                            alt="logo" />
                                    </a>
                                </Link>

                            </div>


                            <div className="header-nav-spacer">
                            </div>


                            <div className="header-nav-menu header-menu-right">

                                <nav className="menu-wrap" role="navigation">
                                    <ul className="menu-list" data-depth="1">

                                        <li className="menu-item" data-type="blog_link" data-url="/blogs/events"
                                            data-collapse-wrap>
                                            <div className="menu-action flex-row flex-middle">

                                                <a className="menu-link" href="/blogs/events" aria-label="EVENTS"
                                                    data-show-spinner>EVENTS</a>

                                            </div>
                                        </li>


                                        <li className="menu-item has-sub">

                                            <div className="menu-action has-sub flex-row flex-middle">
                                                <Link href="/explore">
                                                    <a className="menu-link" aria-label="Explore" aria-haspopup="true">Explore</a>
                                                </Link>
                                            </div>

                                        </li>

                                    </ul>
                                </nav>
                            </div>


                            <div className="header-nav-right flex-row flex-middle">


                                <div className="if-xsmall">
                                    <a className="quick-search-btn header-icon" rel="nofollow" href="/search"
                                        aria-label="Open search form" data-show-modal="#site-search-modal">
                                        <i className="icon icon-search"></i>
                                    </a>
                                </div>


                                <div className="if-small">
                                    <div className="dropdown-wrap" data-dropdown-wrap>
                                        <Link href="/login">
                                            <a className="dropdown-trigger header-icon" rel="nofollow" aria-haspopup="true"
                                                aria-expanded="false" aria-label="Account Links" data-dropdown-trigger>
                                                <i className="icon icon-circle-user"></i>
                                            </a>
                                        </Link>
                                    </div>
                                </div>


                                <div>
                                    <Link href="/cart">
                                        <a className="cart-preview-btn header-icon num-indicator" rel="nofollow"
                                            aria-haspopup="true" aria-expanded="false" aria-label="Cart Preview"
                                            data-menu-toggle="#cart-sidebar-wrap">
                                            <span className="cart-count num-count ">0</span>
                                            <i className="icon icon-cart"></i>
                                        </a>
                                    </Link>
                                </div>

                            </div>

                        </div>
                    </div>
                </nav>
            </header>

        </div>
    )
}

export default Navbar